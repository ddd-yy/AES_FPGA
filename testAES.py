...
def key_expansion(key):
    ...
    for i in range(1, 11):
        ...
        temp = bytes([temp[0] ^ rcon[i-1]] + list(temp[1:]))
        new_key = bytes([a ^ b for a, b in zip(expanded_keys[-1][:4], temp)])
        new_key += bytes([a ^ b for a, b in zip(expanded_keys[-1][4:8], new_key[:4])])
        new_key += bytes([a ^ b for a, b in zip(expanded_keys[-1][8:12], new_key[4:8])])
        new_key += bytes([a ^ b for a, b in zip(expanded_keys[-1][12:], new_key[8:12])])
...
def mix_columns(state):
    ...
    for i in range(0, 16, 4):
        state[i:i+4] = mix_single_column(state[i:i+4])
    return bytes(state)
...
print("Round Keys:")
for i in range(11):
    print(f"Round {i}: {expanded_keys[i].hex()}")
...
for i in range(1, 10):
    ...
    print(f"Round {i} Ciphertext: {state.hex()}")
